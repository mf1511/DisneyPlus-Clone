import React from "react";
import styled from "styled-components";
import("./Header.css");

function Header() {
  return (
    <Nav>
      <Logo src="https://res.cloudinary.com/marin-dev/image/upload/v1643985932/disney%20%2B/images/logo_gzzwvw.svg" />
      <NavMenu>
        <a href="" className="header__link">
          <img
            src="https://res.cloudinary.com/marin-dev/image/upload/v1643985932/disney%20%2B/images/home-icon_x28qr5.svg"
            alt=""
          />
          <span>HOME</span>
        </a>
        <a href="" className="header__link">
          <img
            src="https://res.cloudinary.com/marin-dev/image/upload/v1643985934/disney%20%2B/images/search-icon_zxbanj.svg"
            alt=""
          />
          <span>SEARCH</span>
        </a>
        <a href="" className="header__link">
          <img
            src="https://res.cloudinary.com/marin-dev/image/upload/v1643985937/disney%20%2B/images/watchlist-icon_hsxx2v.svg"
            alt=""
          />
          <span>WATCHLIST</span>
        </a>
        <a href="" className="header__link">
          <img
            src="https://res.cloudinary.com/marin-dev/image/upload/v1643985933/disney%20%2B/images/original-icon_m2dbdn.svg"
            alt=""
          />
          <span>ORIGINALS</span>
        </a>
        <a href="" className="header__link">
          <img
            src="https://res.cloudinary.com/marin-dev/image/upload/v1643985933/disney%20%2B/images/movie-icon_wzh4a0.svg"
            alt=""
          />
          <span>MOVIES</span>
        </a>
        <a href="" className="header__link">
          <img
            src="https://res.cloudinary.com/marin-dev/image/upload/v1643985934/disney%20%2B/images/series-icon_mbvyjm.svg"
            alt=""
          />
          <span>SERIES</span>
        </a>
      </NavMenu>
      <UserImg src="data:image/png;base64," />
    </Nav>
  );
}

export default Header;

const Nav = styled.div`
  height: 80px;
  background-color: #090b13;
  display: flex;
  justify-content: space-between;
  align-items: center;
  padding: 0 36px;
  overflow-x: hidden;
`;

const Logo = styled.img`
  width: 100px;
  display: flex;
  justify-content: center;
  align-items: center;
`;

const NavMenu = styled.div`
  display: flex;
  align-items: center;
  a {
    display: flex;
    justify-content: center;
    align-items: center;
    padding: 0 15px;
    cursor: pointer;
    img {
      height: 25px;
    }
    span {
      font-size: 16px;
      letter-spacing: 1.42px;
      margin-left: 2px;
      position :relative;
      
       &:after {
              content: "";
              height: 2px;
              background: white;
              left: 0;
              right:0;
              bottom: -6px;
              position absolute;
              opacity:0;
              transform-origin: center;
              transition: all 250ms cubic-bezier(0.25, 0.46, 0.45, 0.94) 0s;
              transform:scaleX(0);
            }
    }
    &:hover{
        span:after{
            transform: scaleX(1);
            opacity:1;
        }
    }

  
`;

const UserImg = styled.img`
  width: 50px;
  heigth: 50px;
  border-radius: 50%;
  cursor: pointer;
`;
